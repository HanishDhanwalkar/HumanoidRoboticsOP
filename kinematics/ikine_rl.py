...
class RoboticArmEnv(gym.Env):
    def __init__(self):
        ...
        self.desired_position = np.array([0, 6], dtype=np.float64)  # Desired end-effector position
    ...
    def reset(self):
        ...
        self.desired_position = np.array([0, 6], dtype=np.float32)
    ...
    def render(self):
        # Plot the current configuration of the robotic arm
        plt.figure()
        plt.plot([0, self.arm_length[0]*np.cos(self.joint_angles[0])],
                 [0, self.arm_length[0]*np.sin(self.joint_angles[0])], 'r-')
        plt.plot([self.arm_length[0]*np.cos(self.joint_angles[0])],
                 [self.arm_length[0]*np.sin(self.joint_angles[0])], 'ko')
        plt.plot([self.arm_length[0]*np.cos(self.joint_angles[0]),
                  self.arm_length[0]*np.cos(self.joint_angles[0]) + self.arm_length[1]*np.cos(self.joint_angles[0] + self.joint_angles[1])],
                 [self.arm_length[0]*np.sin(self.joint_angles[0]),
                  self.arm_length[0]*np.sin(self.joint_angles[0]) + self.arm_length[1]*np.sin(self.joint_angles[0] + self.joint_angles[1])], 'g-')
        plt.plot([self.arm_length[0]*np.cos(self.joint_angles[0]) + self.arm_length[1]*np.cos(self.joint_angles[0] + self.joint_angles[1])],
                 [self.arm_length[0]*np.sin(self.joint_angles[0]) + self.arm_length[1]*np.sin(self.joint_angles[0] + self.joint_angles[1])], 'ko')
        plt.plot([self.arm_length[0]*np.cos(self.joint_angles[0]) + self.arm_length[1]*np.cos(self.joint_angles[0] + self.joint_angles[1]),
                  self.arm_length[0]*np.cos(self.joint_angles[0]) + self.arm_length[1]*np.cos(self.joint_angles[0] + self.joint_angles[1]) + self.arm_length[2]*np.cos(self.joint_angles[0] + self.joint_angles[1] + self.joint_angles[2])],
                 [self.arm_length[0]*np.sin(self.joint_angles[0]) + self.arm_length[1]*np.sin(self.joint_angles[0] + self.joint_angles[1]),
                  self.arm_length[0]*np.sin(self.joint_angles[0]) + self.arm_length[1]*np.sin(self.joint_angles[0] + self.joint_angles[1]) + self.arm_length[2]*np.sin(self.joint_angles[0] + self.joint_angles[1] + self.joint_angles[2])], 'b-')
        plt.plot([self.arm_length[0]*np.cos(self.joint_angles[0]) + self.arm_length[1]*np.cos(self.joint_angles[0] + self.joint_angles[1]) + self.arm_length[2]*np.cos(self.joint_angles[0] + self.joint_angles[1] + self.joint_angles[2])],
                 [self.arm_length[0]*np.sin(self.joint_angles[0]) + self.arm_length[1]*np.sin(self.joint_angles[0] + self.joint_angles[1]) + self.arm_length[2]*np.sin(self.joint_angles[0] + self.joint_angles[1] + self.joint_angles[2])], 'ko')
        plt.plot(self.desired_position[0], self.desired_position[1], 'rx')
        plt.xlim(-8, 8)
        plt.ylim(-1, 12)
        plt.gca().set_aspect('equal', adjustable='box')
        plt.show(block=False)  # Show plot without blocking execution
        plt.pause(0.2)  # Delay for 0.5 seconds
        plt.close() 
